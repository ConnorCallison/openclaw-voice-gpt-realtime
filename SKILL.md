...
1. Set your credentials in the plugin config (via OpenClaw settings or `openclaw.json`):
   ...
   - `publicUrl` — a public URL that routes to the plugin's server (port 3335 by default)
...
The agent writes a system prompt for the voice AI, dials the number, and the voice AI handles the conversation autonomously — including navigating phone menus (DTMF), detecting voicemail, and reporting the outcome.
...
- Debug mode (`debug: true`) enables call recording, verbose logging, and latency metrics.
